...
def seed():
    ...
    # --- Schemes ---
    schemes_path = os.path.join(DATA_DIR, "prepared", "schemes.json")
    if os.path.exists(schemes_path) and db.query(models.Scheme).count() == 0:
    ...
    # --- FAQs ---
    faqs_path = os.path.join(DATA_DIR, "prepared", "faqs.json")
    if os.path.exists(faqs_path) and db.query(models.FAQ).count() == 0:
