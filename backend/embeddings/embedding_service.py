...
class EmbeddingService:
    ...
    def load_chunks(
        ...
        if self.chunks_file.suffix.lower() == ".jsonl":

            chunks = self._load_corpus_jsonl()

            logger.info(
                "Loaded %d chunk(s) from corpus JSONL",
                len(chunks),
            )
    ...
    def _load_corpus_jsonl(self) -> List[Dict[str, Any]]:
        """Load flattened visual/text corpus records into embedding records.

        ``corpus.jsonl`` keeps provenance at the top level for easy auditing;
        Chroma expects it under ``metadata``. This adapter preserves every
        visual field, including an empty ``image_path`` value where applicable.
        """
        ...
        with self.chunks_file.open("r", encoding="utf-8") as file:
            ...
            for line_number, line in enumerate(file, start=1):
                ...
                try:
                    ...
                    logger.warning(
                        "Skipping malformed corpus line %d: %s",
                        line_number,
                        exc,
                    )
                    continue
                ...
                if not isinstance(record, dict):
                    logger.warning(
                        "Skipping non-object corpus line %d.",
                        line_number,
                    )
                    continue

                text = str(record.get("text") or "").strip()
                layout = str(record.get("layout") or "text")

                # A visual with no OCR text still needs a searchable vector
                # and its image link persisted. The descriptive text provides
                # a stable retrieval anchor without inventing visual content.
                if not text and record.get("image_path"):
                    text = (
                        f"Visual evidence: {layout} in "
                        f"{record.get('source_file', 'document')} "
                        f"on page {record.get('page_number', 'unknown')}."
                    )

                metadata = {
                    key: record.get(key)
                    for key in (
                        "document_id", "source_file", "processor_used",
                        "page_number", "layout", "confidence",
                        "reading_order", "coordinates", "image_path",
                    )
                }
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate embeddings from chunks.json or visual corpus.jsonl."
    )
    parser.add_argument(
        "--chunks-file",
        type=Path,
        default=CHUNKS_FILE,
        help="Input chunks JSON or corpus JSONL file.",
    )
    ...
    service = EmbeddingService(chunks_file=args.chunks_file)
