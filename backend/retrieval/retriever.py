...
class MultilingualRetriever:
    ...
    def retrieve(
        ...
        # Chroma returns arbitrary metadata inside ``metadata``. Expose visual
        # evidence at the result top level as well, without removing metadata,
        # so API callers do not need Chroma-specific knowledge.
        raw_results = [self._attach_visual_metadata(item) for item in raw_results]
    ...
    def retrieve_best_image(
        ...
        """
        Return the single closest indexed image for a query.

        Visual chunks are searched separately because stronger text chunks can
        otherwise fill the normal RAG ``top_k`` before any image is reached.
        """

        query = self.validate_query(query)

        sarvam_match = self._retrieve_best_sarvam_visual(query)

        if sarvam_match is not None:
            return sarvam_match
        ...
        return self._attach_visual_metadata(results[0])

    @staticmethod
    def _retrieve_best_sarvam_visual(
        query: str,
    ) -> Optional[Dict[str, Any]]:
        """Rank existing Sarvam chart captions when they are not in Chroma."""

        stop_words = {
            "and", "are", "for", "from", "how", "the", "this",
        ...
        for corpus_path in data_dir.glob("sarvam*/corpus.jsonl"):
            try:
                lines = corpus_path.read_text(encoding="utf-8").splitlines()
            except OSError:
                logger.warning("Could not read Sarvam corpus: %s", corpus_path)
                continue
            ...
            for line in lines:
                ...
                # Require at least two meaningful query terms. This prevents a
                # generic Sarvam chart from overriding an unrelated question.
                ...
                source_page_match = re.search(
                    r"Source page:\s*(\d+)",
                    text,
                    flags=re.IGNORECASE,
                )
                page_number = (
                    int(source_page_match.group(1))
                ...
                best_result = {
                    ...
                    "layout": (
                        "chart"
                        if "charts" in Path(image_path).parts
                        else chunk.get("layout", "image")
                    ),
                    "coordinates": chunk.get("coordinates") or {},
                    "metadata": {
                        "chunk_id": chunk.get("chunk_id"),
                        "source_file": chunk.get("source_file"),
                        "page_number": page_number,
                        "processor_used": "sarvam",
                        "image_path": image_path,
                        "layout": "chart",
                        "coordinates": chunk.get("coordinates") or {},
                    },
    ...
    @staticmethod
    def _attach_visual_metadata(result: Dict[str, Any]) -> Dict[str, Any]:
        """Copy visual fields from persisted metadata to a retrieval result."""
        ...
        for key in ("image_path", "layout", "coordinates"):
            value = metadata.get(key)
            if key == "coordinates" and isinstance(value, str):
                try:
                    value = json.loads(value)
                except json.JSONDecodeError:
                    logger.warning("Invalid coordinates metadata for chunk %s", result.get("chunk_id"))
                    value = None
            enriched[key] = value
        return enriched
    ...
    def build_context(
        ...
        for result in results:
            ...
            # Let the LLM know a chart/table/image exists for this
            # source, so its answer can point the reader at it
            # ("see the chart below") instead of only restating
            # OCR'd numbers as if they were the whole picture.
            layout = metadata.get("layout", "text")

            if metadata.get("image_path") and layout != "text":

                source_block += (
                    f"[This source includes an original {layout} "
                    "image, shown separately to the user]\n"
                )
            ...
            sources.append(
                {
                    ...
                    # NOTE: these two were missing entirely before --
                    # even though ingestion had already saved a
                    # cropped chart/table/image and stored its path
                    # in the chunk's metadata, build_sources() never
                    # read it back out, so it never reached the API
                    # response or the CLI printout.
                    "layout": (
                        metadata.get(
                            "layout",
                            "text",
