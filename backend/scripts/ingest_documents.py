...
# Corpus Building
# =============================================================================
def _is_probably_logo(image_path: str) -> bool:
    """Skip attaching small/banner-shaped images (letterheads, logos)
    to text chunks -- these aren't real page content."""
    try:
    ...
    with open(corpus_path, "w", encoding="utf-8") as out:
        ...
        for result in results:
            ...
            for page in document.pages:
                ...
                page_image_path = next(
                    (
                        b.image_path for b in page.blocks
                        if b.image_path and not _is_probably_logo(b.image_path)
                    ),
                    None,
                ) if len(page.blocks) <= 2 else None
                ...
                for block in page.blocks:
                    ...
                    chunk = {
                        ...
                        "image_path": (
                            block.image_path
                            if (block.image_path and not _is_probably_logo(block.image_path))
                            else (page_image_path if block.layout == "text" else None)
                        ),
                    }
...
def flatten_json_value(value, prefix=""):
    ...
    if isinstance(value, str):
        ...
        for item in value:
            yield from flatten_json_value(item, prefix)
        ...
        for key, val in value.items():
            ...
            if isinstance(val, (dict, list)):
                parts.extend(flatten_json_value(val, key))
            else:
                parts.append(f"{key}: {val}")
        if parts:
            joined = ". ".join(parts)
            yield f"{prefix} - {joined}" if prefix else joined


def texts_from_json_file(path: Path) -> List[str]:
    ...
    if isinstance(data, list):
        for record in data:
            ...
            if "question" in record and "answer" in record:
                texts.append(f"Q: {record['question']}\nA: {record['answer']}")
                continue
        ...
        scheme_name = str(
            data.get("scheme_name") or data.get("page_title")
            or data.get("page_name") or path.stem
        )
        prefix = f"[{scheme_name}] "
        summary_parts = [scheme_name]
        if data.get("eligibility"):
            summary_parts.append("Eligibility: " + " ".join(str(i) for i in data["eligibility"] if i))
        if data.get("benefits"):
            summary_parts.append("Benefits: " + " ".join(str(i) for i in data["benefits"] if i))
        if data.get("required_documents"):
            summary_parts.append("Required Documents: " + " ".join(str(i) for i in data["required_documents"] if i))
        if len(summary_parts) > 1:
            texts.append(". ".join(summary_parts)[:2000])

        sections = data.get("sections") or {}
        if isinstance(sections, dict):
            for heading, paras in sections.items():
                body = " ".join(p for p in paras if p) if isinstance(paras, list) else str(paras)
                if body.strip():
                    texts.append(f"{prefix}{heading}: {body}")

        for field in ("eligibility", "benefits", "required_documents"):
            items = data.get(field)
            if isinstance(items, list) and items:
                joined = " ".join(str(i) for i in items if i)
                if joined.strip():
                    texts.append(f"{prefix}{field.replace('_', ' ').title()}: {joined}")

        faq = data.get("faq")
        if isinstance(faq, list):
            for item in faq:
                if isinstance(item, dict) and item.get("question"):
                    texts.append(f"{prefix}Q: {item.get('question')}\nA: {item.get('answer', '')}")
        ...
        if isinstance(tables, list):
            for table in tables:
                if not table:
                    continue
                rows_text = " | ".join(" ".join(str(cell) for cell in row) for row in table if row)
                if rows_text.strip():
                    texts.append(f"{prefix}Table: {rows_text}")

        for field in ("contacts", "helpline", "helplines", "officials", "downloads", "links"):
            val = data.get(field)
            if val:
                for line in flatten_json_value(val, field):
                    texts.append(f"{prefix}{line}")

        if not texts:
            texts.extend(flatten_json_value(data, scheme_name))

    return [t for t in texts if t and t.strip()]


def texts_from_csv_file(path: Path) -> List[str]:
    texts: List[str] = []
    with path.open("r", encoding="utf-8-sig") as f:
        reader = csv.DictReader(f)
        for row in reader:
            parts = [f"{k}: {v}" for k, v in row.items() if v not in (None, "", "NA")]
            if parts:
                texts.append(". ".join(parts))
    return texts


def append_json_csv_corpus(input_dir: Path, corpus_path: Path) -> int:
    if not input_dir.exists():
        logger.warning("%s does not exist; skipping JSON/CSV ingestion.", input_dir)
        return 0

    json_files = sorted(input_dir.glob("*.json"))
    csv_files = sorted(input_dir.glob("*.csv"))

    logger.info("Found %d JSON and %d CSV file(s) in %s", len(json_files), len(csv_files), input_dir)

    chunk_count = 0

    with corpus_path.open("a", encoding="utf-8") as out:
        for path, texts_fn, layout in (
            *[(p, texts_from_json_file, "json") for p in json_files],
            *[(p, texts_from_csv_file, "csv") for p in csv_files],
        ):
            ...
            for i, text in enumerate(texts):
                chunk = {
                    "chunk_id": f"{document_id}_j{i}",
                    "document_id": document_id,
                    "source_file": path.name,
                    "processor_used": "json_csv_ingest",
                    "page_number": None,
                    "layout": layout,
                    "text": text,
                    "confidence": 1.0,
            ...
            logger.info("Processed %s -> %d chunk(s)", path.name, len(texts))

    logger.info("Appended %d JSON/CSV chunk(s) -> %s", chunk_count, corpus_path)
    return chunk_count
...
def ingest(
    ...
    force_processor: Optional[str] = None,
    fallback_to_pymupdf: bool = True,
    json_csv_dir: Path = Path("data/raw_json"),
) -> List[RouteResult]:
    ...
    corpus_path = build_corpus(output_dir, results)

    append_json_csv_corpus(json_csv_dir, corpus_path)
...
def main() -> None:
    ...
    parser.add_argument(
        "--json-csv-dir", default="data/raw_json",
        help="Folder of JSON/CSV files to append into the same corpus (default: data/raw_json)",
    )
    ...
    ingest(
        ...
        json_csv_dir=Path(args.json_csv_dir),
    )
