...
export default function LoginPage() {
  ...
  return (
    <div className="max-w-md mx-auto px-5 py-16">
      ...
      >
        <h1 className="font-display text-2xl font-bold text-maroon-dark text-center">Welcome back</h1>
        <p className="text-sm text-maroon-dark/60 text-center mt-1">Choose how you'd like to continue</p>

        <div className="grid grid-cols-3 gap-2 mt-6">
          {[
            { key: "user" as Role, label: "User", icon: User },
            { key: "admin" as Role, label: "Admin", icon: ShieldCheck },
            { key: "guest" as Role, label: "Guest", icon: UserRound },
          ].map((r) => {
            const Icon = r.icon;
        ...
        {role === "guest" ? (
          <div className="mt-7">
            <p className="text-sm text-maroon-dark/60 text-center">
              Browse schemes, chat, and generate checklists without an account.
              Some features (like saved history) need a full account.
            </p>
            <button
          ...
          <div className="mt-7 space-y-4">
            <div>
              <label className="text-sm font-medium text-maroon-dark">Mobile Number</label>
              <input
                type="tel"
                maxLength={10}
                value={mobile}
                onChange={(e) => setMobile(e.target.value.replace(/\D/g, ""))}
                placeholder={role === "admin" ? "9999999999" : "10-digit mobile number"}
                className="w-full mt-1 border border-blush/60 rounded-lg px-3 py-2 text-sm outline-none focus-visible:border-rose"
              />
            </div>
            <div>
              <label className="text-sm font-medium text-maroon-dark">Password</label>
              <input
                type="password"
                value={password}
