...
import { 
  ...
  MessagesSquare, Star, ShieldCheck, Activity, HelpCircle, Wheat,
  FileText, Mic, UserCircle
} from 'lucide-react'
      ...
      <div className="flex items-center justify-between gap-5 mb-10">
        <Link href="/" className="flex items-center gap-3 text-white">
          ...
          <div>
            ...
            <small className="block -mt-0.5 text-white/45 text-[11px]">{t('citizen_welfare_assistant')}</small>
          </div>
        </Link>
        <button onClick={onClose} aria-label={t('close')} className="w-[45px] h-[45px] flex-shrink-0 grid place-items-center border border-white/15 rounded-[14px] bg-white/5 text-white hover:bg-[#ff6b35] hover:border-[#ff6b35] hover:rotate-[6deg] transition-all">
          <X size={20} />
        </button>
      </div>
            ...
            return (
              ...
              >
                ...
                <span className="flex-1 min-w-0 flex flex-col">
                  <strong className="text-sm font-bold">{t(item.label)}</strong>
                  {item.description && <small className="mt-0.5 text-white/40 text-[11px]">{t(item.description)}</small>}
                </span>
            ...
            {services.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 min-w-0 flex flex-col">
                    <strong className="text-sm font-bold">{t(item.label)}</strong>
                    <small className="mt-0.5 text-white/40 text-[11px]">{t(item.description)}</small>
                  </span>
                </Link>
              )
            })}
