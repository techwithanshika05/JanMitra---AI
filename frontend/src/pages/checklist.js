...
export default function Checklist() {
  ...
  const handleServiceChange = async (e) => {
    ...
    try {
      ...
      const response = await api.generateChecklist({ service_type: id })
      setChecklist({
        name: selected?.name || response.service_type,
        processing_time: response.estimated_time,
        fee: selected?.fee || 'Confirm with department',
        notes: response.notes,
        steps: response.steps || [],
        documents: response.documents.map(name => ({
          name,
          mandatory: true,
          note: 'Required by the generated backend checklist'
        }))
      })
      setCheckedItems([])
    } catch (error) {
      setStatusMessage(error.message)
      setChecklist(documentData[id] || null)
    } finally {
      setLoading(false)
    }
  }
  ...
  const saveChecklist = async () => {
    if (!checklist || !selectedService) return
    setLoading(true)
    setStatusMessage('')
    try {
      const response = await api.createSavedChecklist({
        service_id: selectedService,
        service_name: checklist.name,
        language: 'en'
      })
      setStatusMessage(`Saved successfully (${response.storage_mode} storage).`)
    } catch (error) {
      setStatusMessage(error.message)
    } finally {
      setLoading(false)
  ...
  const handleDownloadPDF = async () => {
    ...
    setStatusMessage('')
    try {
      const blob = await api.downloadChecklistPdf({ service_type: selectedService })
      const url = URL.createObjectURL(blob)
      const anchor = document.createElement('a')
      anchor.href = url
      anchor.download = `${selectedService}_checklist.pdf`
      document.body.appendChild(anchor)
      anchor.click()
      anchor.remove()
      URL.revokeObjectURL(url)
      setStatusMessage('Backend-generated PDF downloaded.')
    } catch (error) {
      setStatusMessage(error.message)
    } finally {
      setLoading(false)
      ...
      <section className="grid grid-cols-1 lg:grid-cols-[minmax(300px,0.78fr)_minmax(0,1.65fr)] bg-[#f4f0e6] dark:bg-[#111713] rounded-b-[12px] overflow-hidden">
        ...
        <aside className="p-8 sm:p-[55px_42px] bg-[#ebe5d7] dark:bg-[#19211c] border-r border-[#d4cebf] dark:border-[#303830]">
          ...
          <div>
            ...
            <p className="text-[0.96rem] leading-relaxed text-[#6d706a] dark:text-[#94a3b8]">{t('select_service_help')}</p>
          </div>
          ...
          <div className="relative mt-12">
            ...
            <div className="relative z-10 flex items-start gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-[0.8rem] leading-relaxed text-[#7c7c73] dark:text-[#94a3b8]">{t('tell_us_applying')}</p>
              </div>
            </div>
            <div className="relative z-10 flex items-start gap-4 mb-6">
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row items-start justify-between gap-7 pb-8 border-b-2 border-[#17211b] dark:border-[#edf7f4]">
                ...
                <div className="flex gap-2">
                  <button onClick={saveChecklist} disabled={loading} className="min-h-[45px] px-4 flex items-center justify-center gap-2 border border-[#0b6847] bg-[#0b6847] text-white text-[0.78rem] font-bold disabled:opacity-50">
                    <BadgeCheck size={16} />
                    <span>{t('save_checklist')}</span>
                  </button>
              ...
              <div className="my-9 grid grid-cols-[120px_1fr] border border-[#d7d3c8] dark:border-[#303830] bg-[#f7f4eb] dark:bg-[#19201b]">
                <div className="p-[24px_20px] flex flex-col justify-center bg-[#0b6847] dark:bg-[#1a5a3e] text-white">
                  ...
                  <span className="mt-1 text-[0.61rem] font-extrabold tracking-[0.13em] text-[#dff45f]">{t('ready')}</span>
                </div>
              ...
              <div className="flex items-end justify-between mb-4">
                <div>
                  ...
                  <h3 className="mt-1.5 text-[1.55rem] tracking-[-0.035em] text-[#17211b] dark:text-[#edf7f4]">{t('required_docs')}</h3>
                </div>
              ...
              <div className="mt-9 pt-5 flex gap-3 border-t border-dashed border-[#c6c3b9] dark:border-[#303830] text-[#7e8079] dark:text-[#94a3b8]">
                ...
                <div className="text-[0.76rem] leading-relaxed">
                  <p>{checklist.notes || 'Requirements may vary depending on your case. Confirm important details with the respective department before submission.'}</p>
                  {checklist.steps?.length > 0 && <ol className="mt-3 list-decimal pl-5 space-y-1">{checklist.steps.map(step => <li key={step}>{step}</li>)}</ol>}
                  {statusMessage && <p className="mt-3 font-bold text-[#0b6847]">{statusMessage}</p>}
                </div>
              </div>
            </>
          )}
