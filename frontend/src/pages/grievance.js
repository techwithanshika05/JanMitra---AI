...
export default function Grievance() {
  ...
  const categories = [
    { id: 'ration_denial', icon: ShoppingBasket, label: 'Ration Denied', desc: 'Ration refused, reduced or unavailable' },
    { id: 'scheme_not_received', icon: IndianRupee, label: 'Scheme Failed', desc: 'Eligible benefit has not been received' },
    { id: 'corruption', icon: ShieldAlert, label: 'Corruption', desc: 'Bribery or misconduct by an official' },
    { id: 'pension_stopped', icon: CirclePause, label: 'Pension Stopped', desc: 'Pension delayed, stopped or not credited' },
  ]
  ...
  const loadSteps = async (catId) => {
    setLoading(true)
    try {
      const res = await fetch(`/api/grievance/category/${catId}`)
      const data = await res.json()
      if (data.success) setSteps(data.category)
    } catch (e) {
      // silent
    }
    setLoading(false)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/grievance/generate', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        document.getElementById('complaint-output')?.scrollIntoView({ behavior: 'smooth' })
      }
    } catch (e) {
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <option value="Pension Stopped or Not Received">Pension Stopped</option>
                      <option value="Other PDS/Welfare Issue">Other Issue</option>
                    </select>
